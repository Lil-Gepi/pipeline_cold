...
<(samtools view -T "$ref_fa" -h "$in_folder_329/a/LB_329.D707.cram" | samtools sort -O SAM -|./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$read_length_329 ) \
<(samtools view -T "$ref_fa" -h "$in_folder_329/b/LB_329.D707.cram" | samtools sort -O SAM -|./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$read_length_329 ) \
<(samtools view -T "$ref_fa" -h "$in_folder_356/a/LB_356.D707+D507.cram" | samtools sort -O SAM -|./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$read_length_356 )|\
