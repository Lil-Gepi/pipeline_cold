...
/^#/{
	...
	if (fmt_done==0 && in_format==1) {
		...
		print "##FORMAT=<ID=PL,Number=G,Type=Integer,Description=\"List of Phred-scaled genotype likelihoods\">"
		print "##FORMAT=<ID=SAD,Number=1,Type=Integer,Description=\"Sum of allele depths, SAD = sum(AD)\">"
...
}
{
	...
	# indices of FMT/AD, FMT/QS, and FMT/DP
	adx=0
	qsx=0
        dpx=0
	split(FORMAT,x,":")
	for (i in x)
        ...
        if (and(adx,qsx,dpx)) {
                print "One of FORMAT/DP, FORMAT/AD, or FORMAT/QS is missing, cannot proceed." > "/dev/stderr"
		exit 1
        }
	...
	x[sadx]="SAD"
        x[afx]="AF"
	x[xfx]="XF"
        # also prepend a placeholder genotype and likelihood
        x[0]="GT:PL"
	FORMAT=collapse(x,":")
	...
	for (i=10;i<=NF;i++) {
		...
		x[afx]=collapse(af,",")
		x[xfx]=collapse(xf,",")
                # prepend missing GT and PL entries
                x[0]="./.:."
