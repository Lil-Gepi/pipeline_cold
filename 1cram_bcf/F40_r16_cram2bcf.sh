...
out_bcf="${cold_dir}/data/F40_r16/coldF40_r16_raw.bcf"
...
<(samtools view -T "$ref_fa" -h -F 0x400 "$in_dir_1/a/LB_185.D710+D505.cram" | ./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$rl) \
<(samtools view -T "$ref_fa" -h -F 0x400 "$in_dir_1/b/LB_185.D710+D505.cram" | ./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$rl) \
<(samtools view -T "$ref_fa" -h -F 0x400 "$in_dir_1/c/LB_185.D710+D505.cram" | ./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$rl) \
<(samtools view -T "$ref_fa" -h -F 0x400 "$in_dir_1/d/LB_185.D710+D505.cram" | ./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$rl) \
<(samtools view -T "$ref_fa" -h -F 0x400 "$in_dir_1/e/LB_185.D710+D505.cram" | ./rupert_pipe_v2/flag-short.awk -v READ_LENGTH=$rl) |\
...
in_bcf="${cold_dir}/data/F40_r16/coldF40_r16_raw.bcf"
# output path (use - to write uncompressed BCF to stdout, or leave empty to write VCF to stdout)
out_bcf="${cold_dir}/data/F40_r16/coldF40_r16_pre.bcf"
