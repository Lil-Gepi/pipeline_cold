...
cov_freq_r14 <- cbind(Allele_f, Coverage, Alleles)
...
####autosomes----
cov_freq_auto_r14 <- cov_freq_r14[which(cov_freq_r14$chr != "X"),]
for (r in replicates){ 
  print(r)
  ...
  for(j in seq_len(nb_rounds)){
    cov_freq_trial <- sample_n(cov_freq_auto_r14, size = nb_SNPs)
...
####X chromosome----
ne_estimates <- NULL
cov_freq_x_r14 <- cov_freq_r14[which(cov_freq_r14$chr == "X"),]
for (r in replicates){ 
  print(r)
  pref_i <- paste("F", times[1], ".R", r, sep = "") #time point i
  pref_j <- paste("F", times[2], ".R", r, sep = "") #time point j with j>i
  ...
  for(j in seq_len(nb_rounds)){
    cov_freq_trial <- sample_n(cov_freq_x_r14, size = nb_SNPs)
    pi <- unlist(subset(cov_freq_trial, select = paste(pref_i, ".freq", sep = "")))
    pj <- unlist(subset(cov_freq_trial, select = paste(pref_j, ".freq", sep = "")))
    covi <- unlist(subset(cov_freq_trial, select = paste(pref_i, ".cov", sep = "")))
    covj <- unlist(subset(cov_freq_trial, select = paste(pref_j, ".cov", sep = "")))
...
# CMH----
#####autosome----
freq_auto_r14 <- as.matrix(cov_freq_auto_r14[,1:10])
cov_auto_r14 <-  as.matrix(cov_freq_auto_r14[,11:20])
ne_auto_r14 <- as.integer(median_auto[,6])
p_auto_r14_cmh <- ACER::adapted.cmh.test(freq=freq_auto_r14, coverage=cov_auto_r14, 
                                         Ne=ne_auto_r14, gen=c(0,30), repl = 1:5,
                                         IntGen = F, order = 0)
#####x chromosome----
freq_x_r14 <- as.matrix(cov_freq_x_r14[,1:10])
cov_x_r14 <-  as.matrix(cov_freq_x_r14[,11:20])
ne_x_r14 <- as.integer(median_x[,6])
p_x_r14_cmh <- ACER::adapted.cmh.test(freq=freq_x_r14, coverage=cov_x_r14, 
                                         Ne=ne_x_r14, gen=c(0,30), repl = 1:5,
                                         IntGen = F, order = 0)

res_r14_auto <- as.data.frame(cbind(CHR = cov_freq_auto_r14["chr"], BP = cov_freq_auto_r14["pos"], P = p.adjust(p_auto_r14_cmh, method = "fdr"), SNP = row.names(cov_freq_auto_r14)))
res_r14_x <- as.data.frame(cbind(CHR = cov_freq_x_r14["chr"], BP = cov_freq_x_r14["pos"], P = p.adjust(p_x_r14_cmh, method = "fdr"), SNP = row.names(cov_freq_x_r14)))
res_r14 <- rbind(res_r14_x, res_r14_auto)

res_r14 <- res_r14[which(res_r14$P > 1e-50),]
colnames(res_r14) <- c("CHR","BP","P","SNP")
res_r14$BP <- as.numeric(res_r14$BP); res_r14$P <- as.numeric(res_r14$P)
res_r14["CHR"][res_r14["CHR"] == "2L"] <- 2
res_r14["CHR"][res_r14["CHR"] == "2R"] <- 3
res_r14["CHR"][res_r14["CHR"] == "4"] <- 6
res_r14["CHR"][res_r14["CHR"] == "3L"] <- 4
res_r14["CHR"][res_r14["CHR"] == "3R"] <- 5
res_r14["CHR"][res_r14["CHR"] == "X"] <- 1
res_r14$CHR <- as.numeric(res_r14$CHR)

saveRDS(object = res_r14, file = "./res_r14")
# res_r14 <- readRDS("./res_r14")

thres_genome <- 1e-7
thres_suggest <- 5e-7

sig_SNP_cmh <- res_r14[which(res_r14$P < thres_genome), 4]
saveRDS(sig_SNP_cmh, file = "./sig_SNP_cmh_r14")
top_SNP_cmh <- res_r14[order(res_r14$P, decreasing = F),][1:100, 4]
saveRDS(top_SNP_cmh, file = "./top_SNP_cmh_r14")
...
pdf("./Plots/r14_plots/CMH_adapted_r14_F0-F30_fdr_adjusted_rising.pdf", width = 10, height = 6)
ggplot(don, aes(x=BPcum, y=-log10(P))) +
  ggtitle("Adapted CMH test among 5 subreplicates in r14 contrasting F0 and F30")+
